...
with main_col:
    ...
    st.markdown('_AI generated results are not guaranteed to be accurate._  \n_Uploaded files will be sent to OpenAI via API. OpenAI\'s policy (as of 10/21/25) is not to use this data to train their models. See [this page](https://platform.openai.com/docs/guides/your-data) for the most recent privacy information._')
        ...
        with (
            ...
            match file_processing_mode:
                ...
                case 'unstract whisper':
                    ...
                    try:
                        st.session_state['unstract_response_json'] = whisper_pdf_text_extraction(st.session_state['config']['unstract_api_key'], temp_table_path, return_json=True)
                    except TimeoutError as e:
                        st.error('Unstract API request timed out. Please try again later, or try a different file processing mode.')
                        st.markdown('Unstract\'s LLMWhisperer API goes down occasionally due to high demand. check <https://status.unstract.com/> for current status.')
                        raise e
    ...
    if 'wab_items' in st.session_state:
        ...
        wab_df = pd.DataFrame(wab_data, columns=['Item Name', 'Item Number', 'Unit', 'Quantity', 'Cost Data Book Year', 'Weighted Average Bid', 'Calculated Cost', 'cdot_name','wab_index'])
        wab_df['Source'] = False
        wab_df['Delete'] = False

        citation_available = 'unstract_response_json' in st.session_state and st.session_state['unstract_response_json'] is not None
        col_config = {
            ...
            'Delete': st.column_config.CheckboxColumn('Delete', help = 'Check to remove this item from the estimate', width = 25),
            'Source': st.column_config.CheckboxColumn(disabled = not citation_available, help = 'Check to show the source for this item from the estimate', width = 25)
        }
        ...
        st.markdown('You can edit the WAB values and quantities directly in the table above, get citations from the original document, or delete items using the checkbox.')
